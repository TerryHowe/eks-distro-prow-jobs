...
#
# sed -i '' -e "s,TerryHowe/eks-distro-build-tooling,${GITHUB_NAME}/eks-distro-build-tooling," $(grep -rl aws/eks-distro-build-tooling *)
sed -i '' -e "s,${ORIGINAL_DATA_BUCKET},${YOUR_DATA_BUCKET}," $(grep -rl ${ORIGINAL_DATA_BUCKET} * | grep -v customize.sh)
sed -i '' -e "s,${ORIGINAL_PRESUBMIT_BUCKET},${YOUR_PRESUBMIT_BUCKET}," $(grep -rl ${ORIGINAL_PRESUBMIT_BUCKET} * | grep -v customize.sh)
